...
def run_calibration_tool():
    ...
    calib_script = Path(__file__).resolve().parent / 'calibrate_projection.py'
    subprocess.run([sys.executable, str(calib_script)])
...
def run_pipeline(config_path, skip_calib_check=False, force_recalibrate=False):
    ...
    video_path = (PROJECT_ROOT / config['video_path']).resolve()
    motion_dir = (PROJECT_ROOT / config['motion_dir']).resolve()
    output_dir = (PROJECT_ROOT / config['output_dir']).resolve()
    calibration_path = (PROJECT_ROOT / config['calibration_path']).resolve()
    ...
    v_loader = VideoLoader(str(video_path))
    m_loader = MotionLoader(str(motion_dir))
    ...
    if options['generate_visualization']:
        ...
        output_video_path = output_dir / f'viz_{viz_type}.mp4'
        ...
        out = cv2.VideoWriter(str(output_video_path), fourcc, v_loader.fps, 
    ...
    if options['export_synced_json']:
        synced_json_path = output_dir / 'synced_poses.json'
...
def main():
    ...
    if not config_candidate.is_absolute():
        config_candidate = PROJECT_ROOT / config_candidate

    if not config_candidate.exists():
        logger.error(f"Configuration file not found: {config_candidate}")
        logger.error("Please create a config file or use default: config/pipeline_config.json")
        return
    
    run_pipeline(config_candidate, args.skip_calib_check, args.recalibrate)
