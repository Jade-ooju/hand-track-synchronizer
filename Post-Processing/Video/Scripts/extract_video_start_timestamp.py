...
def main():
    ...
    parser.add_argument('--video', default=str(PROJECT_ROOT / 'data' / 'raw' / 'Utensil' / 'Utensil_Retake_001.mp4'),
    ...
    if args.interactive:
        result = interactive_timestamp_extraction(str(video_path))
        if result:
            logger.info(f"\nUse this timestamp in your config: {result}")
    elif args.auto:
        result = find_first_recording_frame(str(video_path))
        if result:
