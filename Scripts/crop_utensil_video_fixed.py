...
def run_cropping():
    ...
    recording_frame, recording_start_sec = find_recording_start(video_path)
    ...
    logger.info("="*70)
    unix_ts = get_unix_timestamp_from_frame(video_path, recording_frame)
    ...
    if unix_ts is None:
        ...
        with open(os.path.join(json_dir, first_json), 'r') as f:
            data = json.load(f)
